...
**Hard rule:** Only 160×160px and 120×120px are valid illustration sizes. If you need a larger visual (e.g., hero imagery, feature showcases), use photography from the `orangelogic-dam` skill — not an upsized illustration. If you need a smaller visual (e.g., inline with text in a card), use an `Icon` component from the `constellation-icons` skill instead — not a downsized illustration. Illustrations at 48px, 64px, 80px, 280px, or any non-standard size are always wrong.
