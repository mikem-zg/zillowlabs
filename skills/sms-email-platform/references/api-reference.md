...
- [Google OAuth2 Authorization](#google-oauth2-authorization)
...
Authenticate users via Google and automatically provision API keys. Only users with emails on allowed domains can complete the flow.
...
|-------|------|----------|-------------|
| redirect_uri | string | No | HTTPS URL to redirect to after authorization. The API key and email are appended as query parameters. |

If `redirect_uri` is omitted, the response is returned as JSON.

#### Flow

1. User is redirected to Google's consent screen
2. User signs in and grants access
3. Platform validates email domain against `GOOGLE_ALLOWED_DOMAINS`
4. Platform creates or links the user account
5. Platform provisions a new API key

#### Redirect Response

If `redirect_uri` is provided, the user is redirected to:

```
https://yourapp.com/callback?api_key=sk_...&user_email=user@domain.com
```

#### JSON Response (no redirect_uri)
...
| Status | Condition |
|--------|-----------|
| 400 | Missing authorization code, invalid state, or no email on Google account |
...
{
  "error": "Access denied",
  "message": "Your email domain is not authorized. Allowed domains: zillowgroup.com"
}
```

#### Environment Variables (platform-side)

| Variable | Description |
|----------|-------------|
| `GOOGLE_CLIENT_ID` | Google OAuth2 client ID |
| `GOOGLE_CLIENT_SECRET` | Google OAuth2 client secret |
| `GOOGLE_ALLOWED_DOMAINS` | Comma-separated list of allowed email domains (e.g., `zillowgroup.com,zillow.com`) |
