...
**Use both** `usage_cost_aggr` and `dbsql_cost_per_query` for cost analysis. For dbt/SQL-heavy projects, warehouse costs from `dbsql_cost_per_query` often dominate.
...
2. **Aggregate daily cost:** Combine **both** sources: (a) `usage_cost_aggr.usage_cost` by date for project jobs; (b) `SUM(query_attributed_dollars_estimation)` from `dbsql_cost_per_query` by `date(start_time)` for project warehouses/jobs. Total daily cost = job compute + warehouse.
...
### databricks_cost_optimization_summary.html (Big Picture)
...
| Cost at a glance | Total spend, job vs warehouse split, top 5–10 cost drivers |
...
### databricks_cost_optimization_report.html (Deep Dive)
...
Both files: semantic HTML, minimal CSS, project root.
