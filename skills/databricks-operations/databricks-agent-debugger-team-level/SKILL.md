---
name: databricks-agent-debugger-team-level
description: >-
...
> **Refresh command:** `bash .agents/skills/databricks-agent-debugger-team-level/refresh.sh`
>
...
- **Purpose**: Profile all agents on a team — routing history, ranking positions, connection delivery, and performance factors over a configurable lookback period.
- **Required Input**: Set `TEAM_ZUIDID` (integer) at the top of the notebook. Optionally adjust `DAYS_BACK` (default 35), `PERIOD_DAYS` (default 30), and `SANDBOX_DB` (default `sandbox_pa.u_bretttr`).
- **Key Outputs**:
  - Per-agent ranking positions and days ranked
  - Connection delivery counts and share
  - Performance factor comparisons across team members
  - Sandbox tables created for the run (stored in `SANDBOX_DB`)
  - Identifies under/over-served agents within the team
- **How to Run**: Submit as a Databricks notebook run. Edit `TEAM_ZUIDID`, then Run All. Post the resulting run URL in chat.
- **Notebook ID**: 2600793814939785
- **Refresh**: `bash .agents/skills/databricks-agent-debugger-team-level/refresh.sh`
...
print(f'Saved to {path}')
  ...
  curl -s -X POST "https://zillowlabs-core.replit.app/skill-used" \
    ...
    -d "{\"skillSlug\": \"databricks-agent-debugger-team-level\", \"replOwner\": \"${REPL_OWNER:-}\"}" > /dev/null 2>&1 &
  ```
