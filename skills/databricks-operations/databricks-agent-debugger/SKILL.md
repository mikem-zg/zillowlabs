...
- **Purpose**: Profile a single agent's 30-day routing history to diagnose why they are/aren't receiving connections.
- **Required Input**: Set `AGENT_ZUID` (integer) at the top of the notebook. Optionally adjust `DAYS_BACK` (default 35) and `PERIOD_DAYS` (default 30).
- **Key Outputs**:
  - Ranking positions (median, P25, P75) and days ranked
  - FindPro call-down results (called %, connected %)
  - Connection share from `routing_cxn_share_new_buckets`
  - Competitor analysis (perf_score trends, capacity penalties)
  - Single inline summary table printed at the end
- **How to Run**: Submit as a Databricks notebook run. Edit `AGENT_ZUID`, then Run All. Post the resulting run URL in chat.
- **4 Sequential Queries**: (1) focal agent ranking rows, (2) FindPro calls on those leads, (3) agent connections, (4) competitor ranking on same leads.
- **Notebook ID**: 2186037508178725
- **Refresh**: `bash .agents/skills/databricks-agent-debugger/refresh.sh`
...
if agents_with_no_data:
  ...
  curl -s -X POST "https://zillowlabs-core.replit.app/skill-used" \
    ...
    -d "{\"skillSlug\": \"databricks-agent-debugger\", \"replOwner\": \"${REPL_OWNER:-}\"}" > /dev/null 2>&1 &
  ```
