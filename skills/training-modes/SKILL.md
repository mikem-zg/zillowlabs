...
1. **Sync code**: `python -m databricks publish notebooks`
2. **Run rapid test**: `python -m databricks rapid`
3. **Review output**: CV metrics (mean MAE ± std), feature importance rankings, train MAE, correlation, zero-importance flags
...
"Test this feature" / "Validate this signal" / "Does this help?"
    ...
    → python -m databricks rapid               (trains direct_reg only, ~1-2 min with shared cluster, ~4-6 min with new cluster)
...
"Quick local sanity check" / "Does the code even run?"
    → python train_zip_model.py                 (reduced params, ~2-3 min)
...
1. **Row count** — the final `zip_hurdle_train_features` table should have rows for all training periods (5 for production, 3 for rapid). If any period is missing, a period task likely failed silently.
2. **Period coverage** — check that `SELECT DISTINCT period FROM sandbox_pa.agent_ops.zip_hurdle_train_features` returns the expected labels (all 5 for production: Oct 2025 through Feb 2026; 3 most recent for rapid: Dec 2025, Jan 2026, Feb 2026).
...
If any of these checks fail, inspect the individual Databricks task outputs in the run. The for-each task shows each period's status independently — look for the failed period.
...
These are reported in `evals_result_` after training and printed to output. They appear in both local training (`train_zip_model.py`) and Databricks training (`zip_hurdle_training.py`).
...
**What it does:**
1. Submits a **standalone one-off run** (via `runs/submit`) with a custom lean task graph — does NOT trigger the full production job
2. Refreshes only the SQL queries needed for training features (skips `shap_explanations`, `prod_agent_zip_predictions`, `prod_agent_total_predictions`, `recommended_targets`)
3. Runs parallelized feature prep on **3 most recent periods** only (configurable via `RAPID_EVAL_PERIOD_COUNT` in `model/constants.py`), then merge
...
- **Photon runtime** (`14.3.x-photon-scala2.12`) — Spark operations run on the Photon engine for faster SQL and data processing
- **Fewer training periods** — 3 instead of 5 (set `RAPID_EVAL_PERIOD_COUNT` in `model/constants.py` to change)
- **Fewer SQL refreshes** — non-essential queries skipped (defined in `RAPID_SKIP_QUERIES` in `databricks/workflow.py`)
- **No scoring or cache refresh** — the task graph stops after training
- **Existing cluster support** — set `DATABRICKS_COMPUTE_MODE=existing_cluster` and `DATABRICKS_CLUSTER_ID=<id>` to target a running shared cluster (e.g. `shared-edge` or `shared-lts`), eliminating ~3-5 min cluster startup time
...
**What it does:**
1. Submits a **standalone one-off run** (via `runs/submit`) — does NOT trigger the full production job
2. **Skips SQL refresh** entirely (uses existing warehouse data)
3. Runs parallelized feature prep on **2 most recent periods** only (configurable via `MINI_EVAL_PERIOD_COUNT`), with `concurrency=1` to avoid cluster resource contention
...
**Runtime notes:** Both production and rapid use Photon runtime (`14.3.x-photon-scala2.12`) for faster Spark operations.
...
```bash
python -m databricks rapid     # Quick iteration — direct_reg only, no scoring (~10 min)
python -m databricks mini      # Pipeline validation — all models, 500-agent sample → _test tables (~22 min)
...
**Command:** `python train_zip_model.py`

**What it does:**
- Loads `data_cache/eval_mktops_train.parquet`
...
3. Update `.agents/skills/training-modes/references/tuning_log.json` with the results (date, experiment, feature count, MAE values, improvement %, best_tree_count)
...
|---------|------|---------------|---------|---------------|
| `python train_zip_model.py` | Local sanity | direct_reg (reduced params) | No | Local only |
| `python scripts/compare_objectives.py` | Objective comparison | direct_reg x3 (MSE/Huber/Asymmetric) | No | None |
| `python -m databricks rapid` | Rapid iteration | direct_reg | No | `_test` tables |
| `python -m databricks trigger` | Production | All 5 models | Full universe | Production tables |
