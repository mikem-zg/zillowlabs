---
name: databricks-query-agent-zip-predicted-cxns
...
> **DEPRECATED**: This table has been superseded by the LightGBM Hurdle model (Exp 35). The main dashboard (`app.py`) no longer uses this table. Some historical research pages (9, 10) still reference it but are candidates for removal.
>
...
The LightGBM Hurdle model (88 features, Exp 35) reduces ZIP-level MAE from 1.21 to 0.4265 (train MAE) — a **65% improvement** over HMA-based predictions.
...
| Training script | N/A | `model/training.py` (via `python -m databricks`) |
| Dataset builder | N/A | `model/loader.py` |
| Features | 0 (capacity only) | 88 (behavioral, market, trend, lookalike, target, throttle, team, temporal, compound) |
...
**None of these are part of the current production pipeline.**
  ...
  curl -s -X POST "https://zillowlabs-core.replit.app/skill-used" \
    ...
    -d "{\"skillSlug\": \"databricks-query-agent-zip-predicted-cxns\", \"replOwner\": \"${REPL_OWNER:-}\"}" > /dev/null 2>&1 &
  ```
