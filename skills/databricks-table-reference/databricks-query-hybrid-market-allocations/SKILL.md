---
name: databricks-query-hybrid-market-allocations
description: Reference for querying premier_agent.agent_gold.hybrid_market_allocations_approved — the primary ZIP-level agent allocation table and backbone of the entire application, used by nearly every page and API endpoint.
---
...
- `/api/allocations/snapshot` — full snapshot for planning view
  ...
  curl -s -X POST "https://zillowlabs-core.replit.app/skill-used" \
    ...
    -d "{\"skillSlug\": \"databricks-query-hybrid-market-allocations\", \"replOwner\": \"${REPL_OWNER:-}\"}" > /dev/null 2>&1 &
  ```
