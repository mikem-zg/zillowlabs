---
name: databricks-query-flex-zip-connections-fcst
description: Reference for querying sandbox_pa.revenue_optimization.flex_zip_connections_fcst — ZIP-level flex connection forecasts used for forecast trend charts and as the AOP baseline in simulation comparisons.
---
...
- `/api/allocations/simulation` — AOP flex forecast baseline for simulation comparison
  ...
  curl -s -X POST "https://zillowlabs-core.replit.app/skill-used" \
    ...
    -d "{\"skillSlug\": \"databricks-query-flex-zip-connections-fcst\", \"replOwner\": \"${REPL_OWNER:-}\"}" > /dev/null 2>&1 &
  ```
