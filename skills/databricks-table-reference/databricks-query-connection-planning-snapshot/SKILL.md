---
name: databricks-query-connection-planning-snapshot
description: Reference for querying sandbox_pa.revenue_optimization.preferred_connection_planning_snapshot — simulation-derived target allocations per partner used in the snapshot planning view and CSV downloads.
---
...
- `/api/allocations/snapshot` — simulation target column
  ...
  curl -s -X POST "https://zillowlabs-core.replit.app/skill-used" \
    ...
    -d "{\"skillSlug\": \"databricks-query-connection-planning-snapshot\", \"replOwner\": \"${REPL_OWNER:-}\"}" > /dev/null 2>&1 &
  ```
