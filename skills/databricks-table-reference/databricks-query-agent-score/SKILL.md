---
name: databricks-query-agent-score
...
**WARNING**: The main `agent_score` table is a **daily-refreshed snapshot** — it only contains the latest date. For historical lookups, use `premier_agent.agent_silver.agent_score_historical` which has daily snapshots from 2022-03-07 onward (37.8M rows, 1,476 dates). Same schema as `agent_score`.

There is also `agent_score_snapshots` (SCD2 with dbt_valid_from/to) but it only goes back ~25 days.

**New agent starting score**: New agents start with a median score of ~0.855 (avg 0.81), not 0 or 1. The score is a weighted composite of cvr_pct_to_market, pickup_rate, zhl_pre_approval_rate, and operational_health_rating_numeric.
...
```
  ...
  curl -s -X POST "https://zillowlabs-core.replit.app/skill-used" \
    ...
    -d "{\"skillSlug\": \"databricks-query-agent-score\", \"replOwner\": \"${REPL_OWNER:-}\"}" > /dev/null 2>&1 &
  ```
