...
### 0. Prefer Serverless Compute (instant startup, no cluster config)

**Serverless is the default for rapid/mini jobs.** Zero provisioning time, no cluster configuration, no AWS capacity issues.

```python
payload = {
    "name": "My Job",
    "tasks": [
        {"task_key": "task1", "environment_key": "Default", "notebook_task": {...}},
    ],
    "environments": [{
...
Key points:
- Tasks use `"environment_key": "Default"` instead of `"job_cluster_key"`
- No `job_clusters` needed — no cluster provisioning at all
- Dependencies (like `lightgbm`) go in the `environments[].spec.dependencies` list
- Starts in ~5 seconds vs 5-10 minutes for dedicated clusters
- Works with Unity Catalog tables
- **Use dedicated clusters only for**: hyperparameter tuning with SparkTrials, very large distributed workloads
...
## Critical Lessons (Exp 36, March 2026)

### 10. Use `USER_ISOLATION` for Unity Catalog access

When job clusters need to read/write Unity Catalog tables, `data_security_mode` must be `USER_ISOLATION` (not `SINGLE_USER`). `SINGLE_USER` mode causes permission errors when the cluster tries to access UC-managed tables, even when the submitting user has access.

```python
"data_security_mode": "USER_ISOLATION",
```

### 11. Use DBR 15.4+ for Delta table compatibility

Tables written by newer Databricks runtimes use Delta protocol features not supported by older runtimes. If you see `DeltaUnsupportedOperationException` or Delta reader version errors, upgrade the cluster's `spark_version`. DBR 15.4 (`15.4.x-scala2.12`) is the current recommended version.

```python
"spark_version": "15.4.x-scala2.12",
```

### 12. Enable Arrow for large pandas → Spark conversions

When converting large pandas DataFrames to Spark DataFrames via `spark.createDataFrame()`, the default RPC serialization hits a 256MB message size limit. Enable Arrow-based conversion:
...
**Warning:** Even with Arrow, converting a ~380K-row DataFrame on a single-node cluster takes ~130 minutes. The bottleneck is the Arrow serialization itself, not disk I/O. For large datasets, prefer pure Spark SQL joins to avoid the pandas roundtrip entirely.

### 13. Mid-run timeout management via API

If a running task is about to exceed its `timeout_seconds`, you can update the live job definition via the Jobs API and the new timeout takes effect for the currently running task:

```python
requests.post(f"{host}/api/2.1/jobs/update", headers=headers, json={
    "job_id": job_id,
    "new_settings": {
        "tasks": [{"task_key": "slow_task", "timeout_seconds": new_timeout, ...}]
    }
})
```

This saved a 2.5-hour run from timing out at the old 7200s limit when the merge step needed 7822s. The API reset took effect immediately for the running task.

### 14. Set generous timeouts for merge/serialization tasks

The pandas→Arrow→Spark→Delta pipeline is the slowest step in the workflow. Set timeouts with 2-3x buffer over observed execution times:

| Task | Observed Duration | Recommended Timeout |
|------|------------------|-------------------|
| Feature prep (per period) | ~2-4 min | 600s |
| Merge features | ~130 min (7822s) | 10800s (3 hours) |
| Train model | ~8 min (463s) | 3600s |
| Score universe | ~10-15 min | 5400s |
...
| Unity Catalog `PERMISSION_DENIED` on tables | `data_security_mode: SINGLE_USER` doesn't inherit UC permissions | Use `USER_ISOLATION` mode |
| `DeltaUnsupportedOperationException` or Delta reader error | Cluster runtime too old for Delta table protocol | Upgrade `spark_version` to `15.4.x-scala2.12` |
| `RPC message exceeds 256MB limit` during `createDataFrame` | Pandas→Spark serialization too large | Enable Arrow: `spark.sql.execution.arrow.pyspark.enabled=true` |
